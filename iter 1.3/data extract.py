...
session.writeXYReport(fileName=os.path.join(texts_directory,relative_path+'_deflection centreline.txt'), xyData=(x0, ))
# Mises Stress
odb = session.odbs[absolute_path]
session.writeFieldReport(fileName=os.path.join(texts_directory,relative_path+'_Stresses everywhere.txt'), append=ON, 
