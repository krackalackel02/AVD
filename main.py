...
for LOAD_SIZE in LOAD:
    LOAD_NAME = '_Load_' + str(int(LOAD_SIZE)) + '_' + str(LOAD_SIZE).split('.')[1] if '.' in str(LOAD_SIZE) else '_Load_' + str(int(LOAD_SIZE))
    ...
    for part in MODELS:
        ...
        for MESH_SIZE in MESH:
            ...
            relative_path ='Job-'+PART_NAME+LOAD_NAME+ '-MESH-' + str(int(MESH_SIZE)) + '_' + str(MESH_SIZE).split('.')[1] if '.' in str(MESH_SIZE) else 'Job-'+PART_NAME+ '-MESH-' + str(int(MESH_SIZE))
            runMesh(MESH_SIZE)
            if MESH_SIZE == MESH[-1]:
